...
def run_enkf(forecast_fn, xt_0, Xf_pool, config: EnKFConfig):
    ...
    dict with keys:
        ...
        'Xf_all'  : np.ndarray (T, Nx, Ne) – forecast ensembles at cycle start
        'Xa_all'  : np.ndarray (T, Nx, Ne) – analysis ensembles at cycle start
        'ens_fcst_traj' : np.ndarray (T, Ne, M+1, Nx) – full member trajectories between cycles
        'truth_fcst_traj' : np.ndarray (T, M+1, Nx) – full truth trajectory between cycles
        'diverged' : bool
    """
    cfg = config
    ...
    # Storage — per-cycle scalars and snapshots
    ...
    for k in range(cfg.T):
        ...
        for e in range(cfg.Ne):
            ...
            Xf_k[:, e] = traj_e[-1, :]             # final state → next forecast
...
def plot_ensemble_spaghetti(res, name, palette, cfg, cycle_range=None, var_indices=None):
    ...
    for row, v in enumerate(var_indices):
        ...
        for k in range(k_start, k_end):
            ...
            ax.plot(t_local, ens_mean, color=color, linewidth=1.5, alpha=0.8)
    ...
    legend_elements = [
        ...
        Line2D([0], [0], color=color, linewidth=1.5, label=f'{name} (ens. mean)'),
    ...
    fig.suptitle(f'Ensemble Spaghetti — {name}  |  Ne={cfg.Ne}  |  M={cfg.M}  |  cycles {k_start}–{k_end-1}',
                 fontsize=13)
    plt.tight_layout(); plt.show()
...
def plot_ensemble_spaghetti_multi(results, palette, cfg, cycle_range=None, var_idx=0):
    ...
    axes[-1, 0].set_xlabel('Forecast step (cumulative)')
    fig.suptitle(f'Ensemble Spaghetti Comparison — {VAR_NAMES[var_idx]}  |  Ne={cfg.Ne}  |  M={cfg.M}',
                 fontsize=13)
    plt.tight_layout(); plt.show()
...
def plot_spread_reduction(res, name, palette, cfg, cycle_range=None, var_idx=0):
    ...
    # Top: spread over time within each window
    spread_series = []
    time_series = []
    analysis_spread = []
    forecast_spread = []
    ...
    for k in range(k_start, k_end):
        ...
        # Std across members at each timestep within the window
        std_over_time = np.std(ens_traj[k, :, :, var_idx], axis=0)  # (M+1,)
        axes[0].plot(t_local, std_over_time, color=color, linewidth=1.0, alpha=0.6)

        # Track start (analysis) and end (forecast) spread
        analysis_spread.append(std_over_time[0])
        forecast_spread.append(std_over_time[-1])
    ...
    cycles_plot = np.arange(k_start, k_start + len(analysis_spread))
    width = 0.35
    axes[1].bar(cycles_plot - width/2, analysis_spread, width,
                label='After analysis', color=color, alpha=0.5, edgecolor='black')
    axes[1].bar(cycles_plot + width/2, forecast_spread, width,
...
# Spaghetti: compare all models for each variable (zoomed to first 15 cycles)
for v in range(3):
    plot_ensemble_spaghetti_multi(results_base, surrogates_palette, cfg_base,
                                  cycle_range=(0, 15), var_idx=v)
...
# Spaghetti comparison under stress
for v in range(3):
    plot_ensemble_spaghetti_multi(results_stress, surrogates_palette, cfg_stress,
                                  cycle_range=(0, 15), var_idx=v)
